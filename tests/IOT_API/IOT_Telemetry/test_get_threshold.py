...
@pytest.mark.parametrize('uuid, db_data', devices_with_events(event_type='threshold'))
def test_get_threshold(uuid, db_data, iot_api):
    _to = parser.parse(str(datetime.utcnow().date()))
    _from = (_to - timedelta(days=1))

    api_events = iot_api['threshold'].get(url_param='?deviceId=%s&from=%s&to=%s&size=100' %
    ...
    if api_events:
        ...
        for event in api_events:
            ...
            assert event['telemetryType'] == db_event['criteria_type'], \
                'Fail: uuid: %s, telemetry type: api %s != %s db' % (uuid, event['telemetryType'], db_event['criteria_type'])
